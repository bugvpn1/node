...
install_ppnode() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/bugvpn1/node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/bugvpn1/node/releases/download/${last_version}/ppanel-node-linux-${arch}.zip"
...
EOF
    ...
    curl -o /usr/bin/ppnode -Ls https://raw.githubusercontent.com/bugvpn1/node/master/scripts/ppnode.sh
