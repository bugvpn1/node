...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/bugvpn1/node/master/scripts/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/bugvpn1/node/master/scripts/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/ppnode -N --no-check-certificate https://raw.githubusercontent.com/bugvpn1/node/master/scripts/ppnode.sh
    if [[ $? != 0 ]]; then
        echo ""
